...
public class MeshData
{
    ...
    Vector3[] borderVertices;//vertices that are not being rendered but are stored to prevent seams
    int[] borderTriangles;
    ...
    public MeshData(int verticesPerLine, bool useFlatShading)
    {
        ...
        vertices = new Vector3[verticesPerLine * verticesPerLine];//number of verts is the width * height
        uvs = new Vector2[verticesPerLine * verticesPerLine];
        triangles = new int[(verticesPerLine - 1) * (verticesPerLine - 1) * 6]; //the number of triangles, if confused draw out the vertices and draw the triangles
    ...
    {
        if(vertexIndex < 0)//checking if its border vertex
        {
            borderVertices[-vertexIndex - 1] = vertexPosition;//-vertexIndex because it starts from 1 since vertexIndex is negative for borders subtract 1 to make it start at 0
        }
        else
        {
    ...
    {
        if (a < 0 || b < 0 || c < 0)//border triangles
        {
    ...
    public void ProcessMesh()
    {
        if(useFlatShading)
        {
            Debug.Log("IM TRYING TO FLATSHADE");
            FlatShading();
        }
        else
        {
